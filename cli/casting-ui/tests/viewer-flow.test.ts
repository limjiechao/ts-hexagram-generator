...
// A casting plan whose recorded `recordedMax`es and resolved `Line`s are EXACTLY
// what `performCast` produces for the given picks. The reducer now DERIVES both
...
// recordedMax/line and would no longer survive cast-by-cast replay.
...
describe('flowReducer — splitCommitted advance path', () => {
  it('advances the slot and DERIVES each split (recordedMax + resolved line) itself', () => {
    let state: FlowState = {
      ...initialFlowState('random', null, null),
      mode: 'casting',
      castingPlan: PLAN,
    }
    // Cast 1 — the action carries only the pick; the reducer derives recordedMax = 48.
...
describe('flowReducer — lineRewound', () => {
  ...
  it("clears the current line's casts on a mid-line rewind", () => {
    const state = inCastingMode({
      ...
      partialCasting: [
        [
          { pick: 21, recordedMax: 48 },
          { pick: 17, recordedMax: 43 },
          { pick: 9, recordedMax: 39 },
        ],
        [
          { pick: 22, recordedMax: 48 },
          { pick: 12, recordedMax: 40 },
          { pick: 8, recordedMax: 36 },
        ],
        [{ pick: 19, recordedMax: 48 }, { pick: 14, recordedMax: 42 }, null],
        [null, null, null],
        [null, null, null],
        [null, null, null],
      ],
...
describe('flowReducer — lineRewound resets the per-line algorithm', () => {
  ...
  it('mid-line rewind clears the line and resets the selectable recordedMax to 48', () => {
