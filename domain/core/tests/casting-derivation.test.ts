...
describe('deriveSplit', () => {
  it('derives every field from {pick, recordedMax}', () => {
    expect(deriveSplit({ pick: 24, recordedMax: 48 })).toEqual({
      stalks: 49,
      leftHeap: 24,
      leftPiles: 5,
      leftRemainder: 4,
  ...
  it('tolerates a right heap that empties after suspension (0 piles, 0 remainder)', () => {
    // recordedMax - pick = 0 → the right heap is just the one suspended stalk.
    // This is the degenerate split the input flows exclude (see the
    // `pick = recordedMax - 1` boundary below): `deriveSplit` still derives it
    // faithfully, but no flow ever feeds it a `pick === recordedMax`.
    const d = deriveSplit({ pick: 40, recordedMax: 40 })
    expect(d.rightHeap).toBe(1)
    expect(d.rightPiles).toBe(0)
    expect(d.rightRemainder).toBe(0)
  })

  it('keeps both remainders in 1..4 at the input ceiling pick = recordedMax - 1', () => {
    // The slider / typed / Inquirer / RNG flows all cap the pick at
    // `recordedMax - 1` precisely so the right heap keeps a countable stalk
    // after the suspension (recordedMax - pick = 1 → remainder 1). This is the
    // boundary that guarantees the "no zero remainder" invariant; spot-check it
    // across several recorded maxes.
    for (const recordedMax of [48, 43, 39, 32, 13]) {
      const d = deriveSplit({ pick: recordedMax - 1, recordedMax })
  ...
  it('satisfies conservation 1 + Lrem + Rrem + 4*combinedPiles === stalks', () => {
    const splits: SplitRecord[] = [
      ...
      { pick: 40, recordedMax: 40 },
      { pick: 20, recordedMax: 43 },
      { pick: 16, recordedMax: 35 },
      { pick: 13, recordedMax: 13 },
    ]
